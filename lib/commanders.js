...
const importFrom = (yargs) => {
    ...
    paths.forEach((file) => {
        const filename = file.charAt(0) === '.' ?
            path.join(process.cwd(), file) : file;
        const configName = args.n || path.basename(filename, args.x ? '' : '.json');
        const success = !args.x ? hostConfiger.loadFromFile(path.join(filename)) :
            hostConfiger.buildConfig(configName, hosts2Config.toConfig(path.join(filename)));
        if (!success) return;
